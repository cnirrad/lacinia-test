...
The `home-routes` handler in the `test-app.routes.home` namespace
...
The `home-page` function will in turn call the `test-app.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/test_app/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `test-app.handler` namespace:
...
* Let `mount` know to start the database connection by `require`-ing `test-app.db.core` in some other namespace.
